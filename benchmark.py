...
class CircuitComparer:
    ...
    def pretty(self):
        ...
        s += (
            str(self.gatecount).rjust(8)
            + str(self.t_before).rjust(9)
            + str(self.t_opt).rjust(10)
        )
        s += str(self.tpar).rjust(6) + str(self.t_after).rjust(7)
        s += "{:.2f}".format(self.time_simpl).rjust(12)
        time_extr = (
            "{:.2f}".format(self.time_extr) if self.time_extr is not None else "-"
        )
...
if __name__ == "__main__":
    ...
    dirs = [
        circ_dir / "Arithmetic_and_Toffoli",
