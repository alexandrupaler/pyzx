...
def match_bialg_parallel(
    g: BaseGraph[VT, ET], matchf: Optional[Callable[[ET], bool]] = None, num: int = -1
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if (
            ...
            and v1p == 0
            and (
                (v0t == VertexType.Z and v1t == VertexType.X)
                or (v0t == VertexType.X and v1t == VertexType.Z)
            )
        ):
            v0n = [n for n in g.neighbors(v0) if not n == v1]
            v1n = [n for n in g.neighbors(v1) if not n == v0]
            if all([types[n] == v1t and phases[n] == 0 for n in v0n]) and all(
                [types[n] == v0t and phases[n] == 0 for n in v1n]
            ):
                i += 1
                for v in v0n:
...
def bialg(
    g: BaseGraph[VT, ET], matches: List[MatchBialgType[VT]]
...
def spider(
    g: BaseGraph[VT, ET], matches: List[MatchSpiderType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def unspider(
    g: BaseGraph[VT, ET], m: List[Any], qubit: FloatInt = -1, row: FloatInt = -1
) -> VT:
...
def match_pivot_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        v0a = phases[v0]
        v1a = phases[v1]
        if not ((v0a in (0, 1)) and (v1a in (0, 1))):
            continue
        if g.is_ground(v0) or g.is_ground(v1):
            continue
        ...
        for n in v0n:
            ...
            if types[n] == VertexType.Z and et == EdgeType.HADAMARD:
                pass
...
def match_pivot_gadget(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if any(types[w] != VertexType.Z for w in v0n):
            continue
        if any(types[w] != VertexType.Z for w in v1n):
            continue
...
def match_pivot_boundary(
    g: BaseGraph[VT, ET], matchf: Optional[Callable[[VT], bool]] = None, num: int = -1
) -> List[MatchPivotType[VT]]:
    """Like :func:`match_pivot_parallel`, but except for pairings of
...
def pivot(
    g: BaseGraph[VT, ET], matches: List[MatchPivotType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def match_lcomp_parallel(
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if vt != VertexType.Z:
            continue
        if not (va == Fraction(1, 2) or va == Fraction(3, 2)):
            continue
        ...
        if not all(types[n] == VertexType.Z for n in vn):
            continue
...
def lcomp(
    g: BaseGraph[VT, ET], matches: List[MatchLcompType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def match_ids_parallel(
    g: BaseGraph[VT, ET], vertexf: Optional[Callable[[VT], bool]] = None, num: int = -1
) -> List[MatchIdType[VT]]:
    ...
    while (num == -1 or i < num) and len(candidates) > 0:
        ...
        if (
            g.is_ground(v0)
            and types[v1] == VertexType.BOUNDARY
            or g.is_ground(v1)
            and types[v0] == VertexType.BOUNDARY
        ):
            # Do not put ground spiders on the boundary
            continue
        candidates.discard(v0)
        candidates.discard(v1)
        if g.edge_type(g.edge(v, v0)) != g.edge_type(
            g.edge(v, v1)
        ):  # exactly one of them is a hadamard edge
            m.append((v, v0, v1, EdgeType.HADAMARD))
        else:
            m.append((v, v0, v1, EdgeType.SIMPLE))
        i += 1
    return m


def remove_ids(
    g: BaseGraph[VT, ET], matches: List[MatchIdType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def match_phase_gadgets(g: BaseGraph[VT, ET]) -> List[MatchGadgetType[VT]]:
    ...
    for v in g.vertices():
        if (
            phases[v] != 0
            and phases[v].denominator > 2
            and len(list(g.neighbors(v))) == 1
        ):
    ...
    for par, gad in parities.items():
        if len(gad) == 1:
            ...
            if (
                phases[n] != 0
            ):  # If the phase of the axel vertex is pi, we change the phase of the gadget
                g.scalar.add_phase(phases[v])
                g.phase_negate(v)
                m.append((v, n, -phases[v], [], []))
        else:
            totphase = (
                sum((1 if phases[n] == 0 else -1) * phases[gadgets[n]] for n in gad) % 2
            )
            for n in gad:
                if phases[n] != 0:
...
def merge_phase_gadgets(
    g: BaseGraph[VT, ET], matches: List[MatchGadgetType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def match_supplementarity(g: BaseGraph[VT, ET]) -> List[MatchSupplementarityType[VT]]:
    ...
    while len(candidates) > 0:
        v = candidates.pop()
        if phases[v] == 0 or phases[v].denominator <= 2:
            continue  # Skip Clifford vertices
        neigh = set(g.neighbors(v))
        if not neigh.isdisjoint(taken):
            continue
        par = frozenset(neigh)
        if par in parities:
            ...
            parities[par] = [v]
        for w in neigh:
            if phases[w] == 0 or phases[w].denominator <= 2 or w in taken:
                continue
...
def apply_supplementarity(
    g: BaseGraph[VT, ET], matches: List[MatchSupplementarityType[VT]]
) -> RewriteOutputType[ET, VT]:
...
def match_gadgets_phasepoly(g: BaseGraph[VT, ET]) -> List[MatchPhasePolyType[VT]]:
    ...
    for v in g.vertices():
        ...
        if g.phase(v) != 0 and g.phase(v).denominator == 4:
            if v in targets:
                targets[v].add(frozenset([v]))
            else:
                targets[v] = {frozenset([v])}
    ...
    for v1, t1 in targets.items():
        ...
        if len(s) == 1:
            c = s.pop()
            if any(len(targets[v2]) == 2 for v2 in c):
                continue
        s = t1.difference({frozenset({v1})})
        for c in [d for d in s if not any(d.issuperset(e) for e in s if e != d)]:
            if not all(v2 in targets for v2 in c):
                continue
            if any(v2 < v1 for v2 in c):
                continue  # type: ignore
            a = set()
            for t in c:
                a.update([i for s in targets[t] for i in s if i in targets])
            for group in itertools.combinations(a.difference(c), 4 - len(c)):
                gr = list(group) + list(c)
                b: Set[FrozenSet[VT]] = set()
                for t in gr:
                    b.update([s for s in targets[t] if s.issubset(gr)])
                if len(b) > 7:
    ...
    for groupp, gad in sorted(matches.items(), key=lambda x: len(x[1]), reverse=True):
        if taken.intersection(groupp):
            continue
        m.append(
            (list(groupp), {s: (gadgets[s] if len(s) > 1 else list(s)[0]) for s in gad})
        )
        taken.update(groupp)
...
def apply_gadget_phasepoly(
    g: BaseGraph[VT, ET], matches: List[MatchPhasePolyType[VT]]
) -> None:
    ...
    for group, gadgets in matches:
        for i in range(4):
            ...
            for j in range(i + 1, 4):
                ...
                if f in gadgets:
                    ...
                    phase = 0
                    g.add_edges(
                        [g.edge(n, v), g.edge(v1, n), g.edge(v2, n)], EdgeType.HADAMARD
                    )
                ...
                for k in range(j + 1, 4):
                    ...
                    if f in gadgets:
                        ...
                        phase = 0
                        g.add_edges(
                            [g.edge(*e) for e in [(n, v), (v1, n), (v2, n), (v3, n)]],
                            EdgeType.HADAMARD,
                        )
        ...
        if f in gadgets:
            ...
            phase = 0
            g.add_edges(
                [g.edge(n, v)] + [g.edge(n, w) for w in group], EdgeType.HADAMARD
            )
