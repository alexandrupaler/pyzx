...
def draw(g: Union[BaseGraph[VT, ET], Circuit], labels: bool = False, **kwargs) -> Any:
    ...
    if settings.mode == "shell":
        ...
        if settings.drawing_backend == "d3":
            ...
            raise TypeError(
                "Unsupported drawing backend '{}'".format(settings.drawing_backend)
            )
...
def arrange_scalar_diagram(g: BaseGraph[VT, ET]) -> None:
    ...
    for v in g.vertices():
        if len(list(g.neighbors(v))) == 1:
            ...
            g.vertex_degree(w) > 1 for w in g.neighbors(v)
        ):  # Not part of a phase gadget
            verts.append(v)
            # if rs[v] < min_row: min_row = rs[v]
            if rs[v] in rows_used:
                rows_used[rs[v]].append(v)
            else:
                rows_used[rs[v]] = [v]
...
def draw_matplotlib(
    ...
    if rows is not None:
        ...
        vertices: Iterable[VT] = [
            v for v in g.vertices() if (minrow <= g.row(v) and g.row(v) <= maxrow)
        ]
        edges: Iterable[ET] = [
            e for e in g.edges() if g.edge_s(e) in vertices and g.edge_t(e) in vertices
        ]
    else:
        vertices = g.vertices()
        edges = g.edges()
    ...
    for e in edges:
        ...
        if bend_wire:
            ...
            pth = path.Path(
                [sp, mid, tp], [path.Path.MOVETO, path.Path.CURVE3, path.Path.LINETO]
            )
            ...
            ax.add_line(
                lines.Line2D(
                    [sp[0], tp[0]], [sp[1], tp[1]], color=ecol, linewidth=0.8, zorder=0
                )
            )
        ...
        if h_edge_draw == "box" and et == 2:  # hadamard edge
            ...
            centre = (
                mid[0] - diag / 2 * math.cos(angle + angle2),
    ...
    for v in vertices:
        ...
        if t == VertexType.Z:
            ax.add_patch(
                patches.Circle(p, 0.2, facecolor="green", edgecolor="black", zorder=1)
            )
        elif t == VertexType.X:
            ax.add_patch(
                patches.Circle(p, 0.2, facecolor="red", edgecolor="black", zorder=1)
            )
            ...
            ax.add_patch(
                patches.Circle(p, 0.1, facecolor="black", edgecolor="black", zorder=1)
            )

        if labels:
            plt.text(
                p[0] + 0.25, p[1] + 0.25, str(v), ha="center", color="gray", fontsize=5
            )
        if a:
            plt.text(
                p[0],
...
def draw_d3(
    ...
    graph_id = "".join(
        random_graphid.choice(string.ascii_letters + string.digits) for _ in range(8)
    )
    ...
    nodes = [
        {
            ...
            "vdata": [
                (key, g.vdata(v, key))
                for key in vdata
                if g.vdata(v, key, None) is not None
            ],
        }
        for v in g.vertices()
    ]
    links = [
        {"source": str(g.edge_s(e)), "target": str(g.edge_t(e)), "t": g.edge_type(e)}
        for e in g.edges()
    ]
    graphj = json.dumps({"nodes": nodes, "links": links})

    with open(
        os.path.join(settings.javascript_location, "zx_viewer.inline.js"), "r"
    ) as f:
        library_code = f.read() + "\n"
...
showGraph('#graph-output-{id}',
  ...
  {node_size}, {hbox}, {labels}, '{scalar_str}');
    ...
    if settings.mode == "notebook":
        ...
        """.format(
            graph_id, graphj, str(w), str(h), str(node_size)
        )
...
def pretty_complex(z: complex) -> str:
    ...
    out = ""
    r, arg = cmath.polar(z)
    farg = Fraction(arg / math.pi).limit_denominator(
        64
    )  # this is now a fraction between -1 and 1
...
def matrix_to_latex(m: np.ndarray) -> str:
    ...
    out += s + "\n\\begin{pmatrix}\n"
    out += " \\\\ \n".join(
        [" & ".join(strip_brackets(pretty_complex(a)) for a in row) for row in m]
    )
    out += "\n\\end{pmatrix}\n\\end{equation}"
    return out
...
def graphs_to_gif(graphs: List[BaseGraph], filename: str, frame_duration: float = 0.5):
    ...
    try:
        ...
        raise Exception(
            "This function requires imageio to be installed (try: pip install imageio)."
        )
