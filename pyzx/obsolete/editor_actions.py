...
def pauli_matcher(
    ...
    types = g.types()
    m: List[Tuple[VT, VT]] = []
    paulis = {v for v in candidates if phases[v] == 1 and vertex_is_zx(types[v])}
    if not paulis:
        return m
    while len(candidates) > 0:
        v = candidates.pop()
        if v in paulis and g.vertex_degree(v) == 2:
            continue
        ...
        if (
            (types[v] == types[w] and et == EdgeType.HADAMARD)
            or (
                vertex_is_zx(types[v])
                and types[v] != types[w]
                and et == EdgeType.SIMPLE
            )
            or (
                types[v] == VertexType.H_BOX
                and phases[v] == 1
                and (
                    (et == EdgeType.SIMPLE and types[w] == VertexType.X)
                    or (et == EdgeType.HADAMARD and types[w] == VertexType.Z)
                )
            )
        ):
            m.append((w, v))
...
def pauli_push(
    g: BaseGraph[VT, ET], matches: List[Tuple[VT, VT]]
) -> rules.RewriteOutputType[ET, VT]:
    ...
    for (
        w,
        v,
    ) in matches:  # w is a Pauli and v is the spider we are gonna push it through
...
def bialgebra(
    ...
    for v, w in matches:
        ...
        for n in g.neighbors(v):
            ...
            etab[g.edge(n, v2)] = (
                [1, 0] if g.edge_type(g.edge(n, v)) == EdgeType.SIMPLE else [0, 1]
            )
            new_verts.append(v2)
        ...
        for n in g.neighbors(w):
            ...
            etab[g.edge(n, w2)] = (
                [1, 0] if g.edge_type(g.edge(n, w)) == EdgeType.SIMPLE else [0, 1]
            )
            for v2 in new_verts:
...
operations = {
    ...
    "pivot": {
        ...
        "matcher": lambda g, matchf: rules.match_pivot_parallel(
...
def operations_to_js() -> str:
    ...
    return json.dumps(
        {
            k: {"active": False, "text": v["text"], "tooltip": v["tooltip"]}
