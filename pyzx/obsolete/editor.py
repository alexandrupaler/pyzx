...
def load_js() -> None:
    ...
    text = """<script type="text/javascript">{0}
        ...
        settings.d3_load_string, data1, data2
    )
    display(HTML(text))
...
def graph_to_json(
    ...
    links = [
        {
            "source": int(g.edge_s(e)),  # type: ignore
...
class ZXEditorWidget(widgets.DOMWidget):
    ...
    def _parse_selection(self) -> Tuple[Set[int], Set[Tuple[int, int]]]:
        ...
        edge_set.update(
            [g.edge(v, w) for v in vertex_set for w in vertex_set if g.connected(v, w)]
        )
        ...
        try:
            ...
            for op_id, data in operations.items():
                if data["type"] == MATCHES_EDGES:
                    ...
                    matches = data["matcher"](g, lambda v: v in vertex_set)
    ...
    def _apply_operation(self, change):
        ...
        try:
            ...
            if data["type"] == MATCHES_EDGES:
                ...
                matches = data["matcher"](g, lambda v: v in vertex_set)
            # Apply the rule
            etab, rem_verts, rem_edges, check_isolated_vertices = data["rule"](
                g, matches
            )
            ...
            selection = json.loads(self.graph_selected)
            selection["nodes"] = [
                v for v in selection["nodes"] if v["name"] not in rem_verts
            ]
            selection["links"] = [
                e
                for e in selection["links"]
                if (
    ...
    def _undo_stack_add(self, description: str, js: str) -> None:
        self.undo_stack = self.undo_stack[
            : len(self.undo_stack) - self.undo_position + 1
        ]
    ...
    def undo(self) -> None:
        if self.undo_position == len(self.undo_stack):
            return
        self.undo_position += 1
        description, js = self.undo_stack[len(self.undo_stack) - self.undo_position]
        self.msg.append(
            "Undo {}: {:d}-{:d}".format(
                description, len(self.undo_stack), self.undo_position
            )
        )
