...
def match_hadamards(
    g: BaseGraph[VT, ET], vertexf: Optional[Callable[[VT], bool]] = None
) -> List[VT]:
...
def hadamard_to_h_edge(
    g: BaseGraph[VT, ET], matches: List[VT]
    ...
    g.scalar.add_power(
        len(matches)
    )  # Correct for the sqrt(2) difference in H-boxes and H-edges
    return (etab, rem_verts, [], True)


def match_connected_hboxes(
    g: BaseGraph[VT, ET], edgef: Optional[Callable[[ET], bool]] = None
) -> List[ET]:
    ...
    if edgef is not None:
        candidates = set([e for e in g.edges() if edgef(e)])
    else:
    ...
    while candidates:
        ...
        if g.edge_type(e) != EdgeType.HADAMARD:
            continue
        v1, v2 = g.edge_st(e)
        if ty[v1] != VertexType.H_BOX or ty[v2] != VertexType.H_BOX:
            continue
...
def fuse_hboxes(
    g: BaseGraph[VT, ET], matches: List[ET]
) -> rules.RewriteOutputType[ET, VT]:
...
def match_copy(
    ...
    while len(candidates) > 0:
        ...
        if (
            phases[v] not in (0, 1)
            or types[v] == VertexType.BOUNDARY
            or g.vertex_degree(v) != 1
        ):
            continue
        ...
        if w in taken:
            continue
        tv = types[v]
        if tv == VertexType.H_BOX:
            tv = VertexType.Z  # v is arity 1, so we can treat it as a Z spider
        tw = types[w]
        if tw == VertexType.BOUNDARY:
            continue
        e = g.edge(v, w)
        et = g.edge_type(e)
                ...
                if et == EdgeType.HADAMARD:
                    if tv == VertexType.Z:
                        if phases[v] == 1:
                            copy_type = (
                                VertexType.BOUNDARY
                            )  # We don't actually copy in this case
                        else:
                            copy_type = VertexType.Z
                    else:
                        if phases[v] != 1:
                            continue
...
def apply_copy(
    g: BaseGraph[VT, ET], matches: List[MatchCopyType[VT]]
) -> rules.RewriteOutputType[ET, VT]:
...
def match_hbox_parallel_not(
    ...
    while len(candidates) > 0:
        h = candidates.pop()
        if types[h] != VertexType.H_BOX or phases[h] != 1:
            continue

        for n in g.neighbors(h):
            if g.vertex_degree(n) != 2 or phases[n] != 1:
                continue  # If it turns out to be useful, this rule can be generalised to allow spiders of arbitrary phase here
            v = [v for v in g.neighbors(n) if v != h][0]  # The other neighbor of n
            if not g.connected(v, h):
                continue
...
def hbox_parallel_not_remove(
    g: BaseGraph[VT, ET], matches: List[Tuple[VT, VT, VT]]
) -> rules.RewriteOutputType[ET, VT]:
    ...
    for h, v, n in matches:
        ...
        for w in g.neighbors(h):
            ...
            if types[w] == VertexType.Z and et == EdgeType.SIMPLE:
                continue
            if types[w] == VertexType.X and et == EdgeType.HADAMARD:
                continue
...
def match_par_hbox(
    ...
    groupings: Dict[
        Tuple[FrozenSet[VT], FrozenSet[VT]], Tuple[List[VT], List[VT], List[VT]]
    ] = dict()
    ty = g.types()
    for h in candidates:
        if ty[h] != VertexType.H_BOX:
            continue
        suitable = True
        ...
        for v in g.neighbors(h):
            ...
            if g.edge_type(e) == EdgeType.HADAMARD:
                if ty[v] == VertexType.X:
                    ...
                    w = [w for w in g.neighbors(v) if w != h][
                        0
                    ]  # unique other neighbor
                    if (
                        ty[w] != VertexType.Z
                        or g.edge_type(g.edge(v, w)) != EdgeType.HADAMARD
                    ):
                        suitable = False
                        break
                    neighbors_NOT.add(w)
                ...
                if ty[v] == VertexType.Z:
                    ...
                    if (
                        ty[w] != VertexType.Z
                        or g.edge_type(g.edge(v, w)) != EdgeType.SIMPLE
                    ):
                        suitable = False
                        break
                    neighbors_NOT.add(w)
...
def par_hbox(
    g: BaseGraph[VT, ET], matches: List[TYPE_MATCH_PAR_HBOX]
) -> rules.RewriteOutputType[ET, VT]:
...
def match_par_hbox_intro(
    ...
    groupings: Dict[
        FrozenSet[VT], List[Tuple[VT, List[VT], Set[VT], Set[VT], Set[VT]]]
    ] = dict()
    ty = g.types()
    for h in candidates:
        if ty[h] != VertexType.H_BOX:
        ...
        if group not in groupings:
            groupings[group] = [
                (h, NOTs, neighbors_regular, neighbors_NOT, neighbors_single)
            ]
            continue
        ...
        for (
            ...
            if g.phase(h) != g.phase(h2):
                continue  # TODO: Allow intro rule with different phases.
            if len(neighbors_single) != len(neighbors_single2):
                continue  # TODO: Allow different sets of neighbours here.
            if (
                len(vs) != 1
            ):  # To use the Intro rule, the H-boxes should differ on exactly one position on where there are NOTs.
                continue
            # We have a match!
            v = vs.pop()
            break
...
def par_hbox_intro(
    g: BaseGraph[VT, ET], matches: List[TYPE_MATCH_PAR_HBOX_INTRO]
) -> rules.RewriteOutputType[ET, VT]:
...
def match_hpivot(g: BaseGraph[VT, ET], matchf=None) -> hpivot_match_output:
    ...
    for h in g.vertices():
        if not (
            (matchf is None or matchf(h))
            and g.vertex_degree(h) == 2
            and types[h] == VertexType.H_BOX
            and phases[h] == 1
        ):
            continue
        ...
        v1nn = [
            (phases[v], list(filter(lambda w: w != v1, g.neighbors(v)))) for v in v1h
        ]

        if not (
            all(all(types[v] == VertexType.Z for v in vs) for vs in v0nn)
            and all(all(types[v] == VertexType.Z for v in vs[1]) for vs in v1nn)
            and len(v0b) + len(v1b) <= 1
            and len(v0b) + len(v0h) + 1 == len(v0n)
            and len(v1b) + len(v1h) + 1 == len(v1n)
        ):
            continue
