...
def basic_optimization(
    circuit: Circuit, do_swaps: bool = True, quiet: bool = True
) -> Circuit:
...
class Optimizer(object):
    ...
    def parse_circuit(
        ...
        for g in correction:
            self.circuit.gates.extend(g.to_basic_gates())
        i = 0
        while True:
            ...
            if self.minimize_czs and (
                all(s1 <= s2 for s1, s2 in zip(count, s)) or i >= max_iterations
            ):
                break
            for g in correction:
                self.circuit.gates.extend(g.to_basic_gates())
                ...
                while gs:
                    ...
                    if g.name not in ("CZ", "CNOT"):
                        ...
                        for i, g2 in enumerate(gs[1:]):
                            if (ty == 1 and isinstance(g2, ZPhase)) or (
                                ty == 2 and isinstance(g2, XPhase)
                            ):
                                output.append(g2)
                                remove.append(i)
                            elif g2.name not in ("CZ", "CNOT"):
                                break
                            elif (
                                ty == 1 and (g2.name == "CZ" or g2.control == q)
                            ) or (  # type: ignore
    ...
    def add_cz(self, cz: CZ) -> None:
            ...
            for c, t in [(t1, t2), (t2, t1)]:
                for g in self.available[c]:
                    if g.name == "CNOT" and g.control == c and g.target == t:  # type: ignore[attr-defined]
                        if self.availty[t] == 2:
                            if (
                                g in self.available[t]
                            ):  # The gate is also available on the target qubit
                                found_match = True
                                break
                            else:
                                continue
                        ...
                        for h in list(
                            reversed(self.gates[t][: -len(self.available[t])])
                        ):  # We start looking at the gates behind the Z-like gates
                            if (
                                h.name != "CNOT" or h.target != t
                            ):  # If any of those gates is not a CNOT of the right type, then we stop our search
                                break
                            if (
                                h == g
                            ):  # But if all the previous gates are fine, than we can use this CNOT.
                                found_match = True
                                break
                        if found_match:
                            break
        ...
        if found_match:  # CNOT-CZ = (S* x id)CNOT (S x S)
            ...
            if self.available[t]:  # There are gates of non-commuting type on this qubit
                self.gates[t].insert(
                    -len(self.available[t]), s1
                )  # And hence we must insert these gates at
            ...
            self.gates[t].append(
                s2
            )  # In contrast, these gates appear after the CNOT, necessarily on Z-like phases
            self.available[t].append(
                s2
            )  # and hence can be added at the end of the list
            s3 = S(c)
        ...
        for g in reversed(
            self.available[t1]
        ):  # We try to find a CZ with the same control and target
            if (
                g.name == "CZ" and g.control == t1 and g.target == t2
            ):  # Here it is important that we have normalised all CZs
                found_match = True  # to have cz.control<cz.target
                break
        if found_match:
            if (
                g not in self.available[t2]
            ):  # We still need to check if the CZ is actually available on the other qubit
                found_match = False
            else:
            ...
            if self.availty[t] == 1:  # Try to find anti-match
                ...
                for g in reversed(self.available[c]):
                    if g.name == "CNOT" and g.control == t and g.target == c:
                        ...
                        break
                if (
                    found_match and self.do_swaps
                ):  # We do the CNOT(t,c)CNOT(c,t) = CNOT(c,t)SWAP(c,t) commutation
    ...
    def parse_gate(self, g: Gate) -> None:
        ...
        if g.name in ("CZ", "CNOT"):
            g.control = next(
                i for i in self.permutation if self.permutation[i] == g.control
            )
        ...
        if g.name == "HAD":
            ...
            if (
                len(self.gates[t]) > 1
                and self.gates[t][-2].name == "HAD"
                and isinstance(self.gates[t][-1], ZPhase)
            ):
                g2 = self.gates[t][-1]
            ...
            if (
                g.phase == 1
            ):  # If the resulting phase is a pi, then we record it as a Z gate
                toggle_element(self.zs, t)
                return
            if g.name == "S":  # We might have changed the phase, and therefore
                g.adjoint = (
                    g.phase.numerator != 1
                )  # Need to adjust whether the adjoint is true
            if (
                t in self.hadamards
            ):  # We can't push a phase gate trough a HAD, so we actually place the HAD down
                self.add_hadamard(t)
            if self.availty[t] == 1 and any(
                isinstance(g2, ZPhase) for g2 in self.available[t]
            ):  # There is an available phase gate
                i = next(
                    i
                    for i, g2 in enumerate(self.available[t])
                    if isinstance(g2, ZPhase)
                )  # That we can fuse with the new one
            ...
            if (
                t1 > t2
            ):  # Normalise so that always g.target<g.control (since CZs are symmetric anyway)
                g.target = t1
                g.control = t2
...
def greedy_consume_gates(
    gates: Dict[int, List[Gate]], qubits: int
) -> Tuple[List[Gate], List[HAD]]:
    ...
    while True:
        had_blocked = (
            dict()
        )  # a {qubit: HADgate} dictionary specifying when a HAD blocks further consuming of gates.
        to_be_appended = []  # List of gates that we will add to ``block``.
        available = (
            []
        )  # List of indices of 2-qubit gates to record whether they are available to be added on the other target.
        gatetype = {
            i: 0 for i in range(qubits)
        }  # 0 = Z-type, 1 = X-type, the two sorts of commutation types.
            ...
            for g in gs:
                ...
                if isinstance(g, ZPhase) or g.name == "CZ":  # Z-type gates
                    if gatetype[q] == 1:  # Z-type is available
                        if g.name == "CZ":
                            if (
                                g.index in available
                            ):  # Check whether the target on the other qubit is available
                                to_be_appended.append(g)
                            else:
                                available.append(
                                    g.index
                                )  # Otherwise we postpone until we have checked that later on
                        else:
                            to_be_appended.append(g)
                    else:
                        break  # We have encountered a gate of the wrong type, so we stop delving deeper
                else:  # gate is CNOT
                    if (gatetype[q] == 1 and g.target == q) or (
                        gatetype[q] == 2 and g.control == q
                    ):  # wrong type
                        break
                    else:
        ...
        if (
            to_be_appended
        ):  # We added at least one gate, so we go to the top of the loop to try again.
            continue
        ...
        for q, had in had_blocked.items():
            ...
            left_ty = gatetype[
                q
            ]  # The type of the gates to the left of the HAD. Note that this type must necessarily
            ...
            if g.name == "CZ" or isinstance(g, ZPhase) or (g.control == q):
                if gatetype[q] == 0:
                    left_ty = 2  # If no gate is on the left of the HAD we set the type correspondingly.
                right_ty = 1
            else:
                if gatetype[q] == 0:
                    left_ty = 1
                right_ty = 2
            if left_ty == right_ty:
                continue  # If the types are different, we can't commute things past the HAD into the phase-block.
            for g in gs:
                if g.name == "HAD":
                    break  # If we encounter another HAD, we stop.
                if isinstance(g, ZPhase):
                    if right_ty == 1:
                        continue  # We can't commute a ZPhase past a HAD, but we can keep looking further
                    else:
                        break  # ZPhase is not of type X, so we must stop looking now.
                if (
                    g.name == "CZ" or g.control == q
                ):  # CZ or CNOT with a control on this qubit
                    if right_ty == 2:
                        break
                ...
                if g.index not in available:
                    if (
                        g.name == "CNOT"
                    ):  # We only need to check CNOTs, since CZs must already be in available
                        available.append(
                            g.index
                        )  # (because otherwise they would be behind 2 HADs)
                else:
                    if g not in candidates:
                        candidates.append(g)
        if added_any:
            continue  # Found double Hadamard

        for g in candidates:
            if g.name == "CZ":
                if (
                    g.target in had_blocked and g.index > had_blocked[g.target].index
                ):  # CZ appears after the HAD.
                    q = g.target
                else:
                    q = g.control
                ...
                if q2 in had_blocked and g.index > had_blocked[q2].index:
                    print(g, g.index)
                    raise Exception(
                        "CZ behind two Hadamard gates. This is not supposed to happen"
                    )
                cnot = CNOT(q2, q)
                cnot.index = g.index
                gates[q].remove(g)
                ...
                if (
                    g.target in had_blocked
                    and g.index > had_blocked[g.target].index
                    and g.control in had_blocked
                    and g.index > had_blocked[g.control].index
                ):
...
def phase_block_optimize(
    circuit: Circuit, pre_optimize: bool = True, quiet: bool = True
) -> Circuit:
    ...
    for i, g in enumerate(circuit.gates):
        ...
        if g.name in ("CNOT", "CZ"):
            ...
            if not isinstance(g, ZPhase):
                raise TypeError(
                    "Unknown gate {}. Maybe simplify the gates with circuit.to_basic_gates()?".format(
                        str(g)
                    )
                )
            elif g.phase.denominator not in (1, 2, 4):
                raise TypeError(
                    "This method only works on Clifford+T circuits. This circuit contains a {}".format(
                        str(g)
                    )
                )
            gates[g.target].append(g)
