...
class GraphS(BaseGraph[int, Tuple[int, int]]):
    ...
    def add_vertex_indexed(self, index):
        ...
        if index >= self._vindex:
            self._vindex = index + 1
    ...
    def incident_edges(self, vertex):
        return [
            (vertex, v1) if v1 > vertex else (v1, vertex) for v1 in self.graph[vertex]
        ]
    ...
    def add_to_phase(self, vertex, phase):
        self._phase[vertex] = (
            self._phase.get(vertex, Fraction(1)) + Fraction(phase)
        ) % 2
