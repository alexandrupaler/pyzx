...
class BaseGraph(Generic[VT, ET], metaclass=DocstringMeta):
    ...
    def __init__(self) -> None:
        ...
        self.phase_index: Dict[
            VT, int
        ] = dict()  # {vertex:index tracking its phase for phase teleportation}
    ...
    def copy(self, adjoint: bool = False, backend: Optional[str] = None) -> "BaseGraph":
        ...
        etab = {
            e: g.edge(vtab[self.edge_s(e)], vtab[self.edge_t(e)]) for e in self.edges()
        }
    ...
    def replace_subgraph(
        self, left_row: FloatInt, right_row: FloatInt, replace: "BaseGraph"
    ) -> None:
        ...
        if set(self.qubit(v) for v in qright) != set(
            replace.qubit(v) for v in r_outputs
        ):
        ...
        self.remove_vertices(
            [
                v
                for v in self.vertices()
                if (left_row < self.row(v) and self.row(v) < right_row)
            ]
        )
        self.remove_edges(
            [self.edge(s, t) for s in qleft for t in qright if self.connected(s, t)]
        )
        ...
        for v in replace.vertices():
            if v in r_inputs or v in r_outputs:
                continue
            vtab[v] = self.add_vertex(
                replace.type(v),
                replace.qubit(v),
        ...
        etab = {
            e: self.edge(vtab[replace.edge_s(e)], vtab[replace.edge_t(e)])
            for e in replace.edges()
        }
    ...
    def compose(self, other: "BaseGraph") -> None:
        ...
        for e in other.edges():
            ...
            if not s in inputs and not t in inputs:
                self.add_edge(self.edge(vtab[s], vtab[t]), edgetype=other.edge_type(e))
    ...
    def add_edges(
        self, edges: Iterable[ET], edgetype: EdgeType.Type = EdgeType.SIMPLE
    ) -> None:
    ...
    def add_edge_table(self, etab: Mapping[ET, List[int]]) -> None:
        ...
        add: Dict[EdgeType.Type, List[ET]] = {
            EdgeType.SIMPLE: [],
            EdgeType.HADAMARD: [],
        }  # list of edges and h-edges to add
        ...
        for e, (n1, n2) in etab.items():
            ...
            if conn_type == EdgeType.SIMPLE:
                n1 += 1  # and add to the relevant edge count
            elif conn_type == EdgeType.HADAMARD:
                n2 += 1
            ...
            # Hence, all the other cases have some kind of parallel edge
                ...
                t1 != t2 and vertex_is_zx(t1) and vertex_is_zx(t2)
            ):  # types are ZX & different
                ...
                if t2 == VertexType.H_BOX:  # They are both H-boxes
                    ...
                    if n2 > 1:
                        raise ValueError(
                            "Parallel H-edges between H-box and Z-spider are not supported"
                        )
                    if n1 and n2:
                        ...
                        w = self.add_vertex(
                            VertexType.H_BOX, (q1 + q2) / 2, (r1 + r2) / 2 - 0.5
                        )
                        add[EdgeType.SIMPLE].extend(
                            [self.edge(v1, w), self.edge(v2, w)]
                        )
                        new_type = EdgeType.SIMPLE
                        self.scalar.add_power(
                            -1
                        )  # H-boxes are scaled differently than H-edges, so we compensate with 1/sqrt(2) here.
                    elif n1:
                        new_type = EdgeType.SIMPLE
                    elif n2:
                        new_type = EdgeType.HADAMARD
                    ...
                    if n1 > 1:
                        raise ValueError(
                            "Parallel edges between H-box and X-spider are not supported"
                        )
                    if n1 and n2:
                        # There is no simple way to deal with a parallel H-edge and regular edge
                        # So we simply add a 2-ary H-box to the graph
                    ...
                    raise ValueError(
                        "Unhandled parallel edges between nodes of type (%s,%s)"
                        % (t1, t2)
                    )
            else:
                raise ValueError(
                    "Unhandled parallel edges between nodes of type (%s,%s)" % (t1, t2)
                )
