...
def compute_row_ops(m):
    ...
    # ops_rows = []
    ops_rows = [ops.data[r] for r, row in enumerate(m1.data) if sum(row) == 1]
...
def alt_extract_circuit(
    ...
    while True:
        ...
        cz_mat = Mat2(
            [[0 for i in range(g.qubit_count())] for j in range(g.qubit_count())]
        )
        for v in frontier:
        ...
        for v in frontier.copy():
            ...
            if any(
                w in inputs for w in d
            ):  # frontier vertex v is connected to an input
        ...
        for bs in range(1, blocksize):
            ...
            ops.copy().gauss(full_reduce=True, y=cnots1, blocksize=bs)
