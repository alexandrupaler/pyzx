...
class QASMParser(object):
    ...
    def parse_custom_gate(self, data: str) -> None:
        ...
        if "(" in spec:
            ...
            if spec[i + 1 : j].strip():
                raise TypeError(
                    "Arguments for custom gates are currently"
        ...
        if " " in spec:
            ...
            raise TypeError(
                "Custom gate specification doesn't have any "
    ...
    def parse_command(
        self, c: str, registers: Dict[str, Tuple[int, int]]
    ) -> List[Gate]:
        gates: List[Gate] = []
        name, rest = c.split(" ", 1)
            ...
            if "[" in a:
                ...
                if is_range:
                    if registers[a][1] != dim:
                        raise TypeError(
                            "Error in parsing {}: Register sizes do not match".format(c)
                        )
                else:
                    dim = registers[a][1]
                is_range = True
        ...
        for j in range(dim):
            ...
            if name in self.customgates:
                ...
                if len(argset) != circ.qubits:
                    raise TypeError(
                        "Argument amount does not match gate spec: {}".format(c)
                    )
                for g in circ.gates:
            ...
            if name in ("x", "z", "s", "t", "h", "sdg", "tdg"):
                if name in ("sdg", "tdg"):
                    ...
                    g = qasm_gate_table[name](argset[0])  # type: ignore # - Gate subclasses with different numbers of parameters
                gates.append(g)
                continue
            if (
                name.startswith("rx")
                or name.startswith("rz")
                or name.startswith("u1")
                ...
                elif name.startswith("crz"):
                    g = CRZ(argset[0], argset[1], phase=phase)
                else:
                    g = ZPhase(argset[0], phase=phase)
                gates.append(g)
                continue
            if name.startswith("u2") or name.startswith(
                "u3"
            ):  # see https://arxiv.org/pdf/1707.03429.pdf
                i = name.find("(")
                j = name.find(")")
                if i == -1 or j == -1:
                    raise TypeError("Invalid specification {}".format(name))
                ...
                if name.startswith("u2"):
                    if len(phases) != 2:
                        raise TypeError("Invalid specification {}".format(name))
                    gates.append(
                        ZPhase(argset[0], phase=(phases[1] - Fraction(1, 2)) % 2)
                    )
                    gates.append(XPhase(argset[0], phase=Fraction(1, 2)))
                    gates.append(
                        ZPhase(argset[0], phase=(phases[0] + Fraction(1, 2)) % 2)
                    )
                    continue
                else:
