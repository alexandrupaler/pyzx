    ...
    for gate in gates:
        ...
        if len(l) == 2 and l[1].find("nocontrol") != -1:  # no controls
            if gname == "H":
                ...
                c.add_gate("S", t, adjoint=adjoint)
            elif gname == "T":
                c.add_gate("T", t, adjoint=adjoint)
            else:
        ...
        if ctrls.find(",") != -1:
            if ctrls.count(",") != 1:
                raise TypeError("Maximum two controls on gate allowed: " + gate)
            if gname not in ("not", "Z"):
                raise TypeError("Two controls only allowed on 'not' and 'Z': " + gate)
            c1, c2 = ctrls.split(",", 1)
            ...
            for ts in nots:
                c.add_gate("NOT", ts)
            if gname == "not":
                c.add_gate("TOF", ctrl1, ctrl2, t)
            elif gname == "Z":
                c.add_gate("CCZ", ctrl1, ctrl2, t)
            for ts in nots:
                c.add_gate("NOT", ts)
            continue
        elif ctrls.find("+") == -1:
        ...
        ctrl = int(ctrls[1:])
        if gname == "not":
            c.add_gate("CNOT", ctrl, t)
        elif gname == "Z":
            c.add_gate("CZ", ctrl, t)
        elif gname == "X":
            c.add_gate("CX", ctrl, t)
        else:
