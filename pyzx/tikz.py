...
def _to_tikz(
    ...
    if draw_scalar:
        ...
        s = "        \\node [style=none] ({:d}) at ({:.2f}, {:.2f}) {{{:s}}};".format(
            idoffset, x, y, scalar
        )
        idoffset += 1
        verts.append(s)
    ...
    for v in g.vertices():
        ...
        if ty == VertexType.BOUNDARY:
            ...
            if p != 0:
                if ty == VertexType.Z:
                    style = settings.tikz_classes["Z phase"]
                else:
                    style = settings.tikz_classes["X phase"]
            else:
                if ty == VertexType.Z:
                    style = settings.tikz_classes["Z"]
                else:
                    style = settings.tikz_classes["X"]
        ...
        if ty == EdgeType.HADAMARD:
            if g.type(v) != VertexType.BOUNDARY and g.type(w) != VertexType.BOUNDARY:
                ...
                t = "        \\node [style={:s}] ({:d}) at ({:.2f}, {:.2f}) {{}};".format(
                    settings.tikz_classes["H"], maxindex + 1, x, y
                )
                verts.append(t)
                maxindex += 1
        else:
...
def to_tikz_sequence(
    graphs: List[BaseGraph], draw_scalar: bool = False, maxwidth: FloatInt = 10
) -> str:
    ...
    return TIKZ_BASE.format(
        vertices="\n".join(total_verts), edges="\n".join(total_edges)
    )


def tikzit(
    g: Union[BaseGraph[VT, ET], Circuit, str], draw_scalar: bool = False
) -> None:
    ...
    if not settings.tikzit_location or shutil.which(settings.tikzit_location) is None:
        raise Exception(
            "Please point towards the Tikzit executable"
...
tikz_error_message = (
    "Not a valid tikz picture. Please use Tikzit to generate correct output."
)
...
def tikz_to_graph(
    ...
    for c, l in enumerate(lines[2:]):
        ...
        i = l.find("[")
        if i == -1:
            raise ValueError("Node definition %s does not include style" % l)
        j = l.find("]", i)
        style = l[i + 1 : j].strip()
        style = style.split("=")[1]
        ...
        ty: VertexType.Type
        if style.lower() in synonyms_boundary:
            ty = VertexType.BOUNDARY
        elif style.lower() in synonyms_z:
            ty = VertexType.Z
        elif style.lower() in synonyms_x:
            ty = VertexType.X
        elif style.lower() in synonyms_hadamard:
            ty = VertexType.H_BOX
        else:
            if ignore_nonzx:
                ty = VertexType.BOUNDARY
                ...
                raise ValueError(
                    "Unknown vertex style '%s' in node definition %s" % (style, l)
                )

        if pos in position_dict:
            if warn_overlap:
                raise Warning(
                    "Vertices %d and %s have same position"
                    % (vid, str(position_dict[pos]))
                )
            if fuse_overlap:
                ...
                if label == "" or label == "-" or label == "-1":
                    ...
                    if label.find("}{") == -1:
                        ...
                        try:
                            ...
                            raise ValueError(
                                "Node definition %s has invalid phase label" % l
                            )
                    else:
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Node definition %s has invalid phase label '%s'"
                            % (l, label)
                        )
                    g.set_phase(v, phase)
    ...
    for c, l in enumerate(lines[c + 4 :]):
        ...
        if style.lower() in synonyms_edge:
            ...
            if ignore_nonzx:
                ...
                raise ValueError(
                    "Unknown edge style '%s' in edge definition %s" % (style, l)
                )
