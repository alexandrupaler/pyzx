...
def json_to_graph(js: str) -> BaseGraph:
    ...
    for name, attr in j.get("node_vertices", {}).items():
        if (
            "data" in attr
            and "type" in attr["data"]
            and attr["data"]["type"] == "hadamard"
            and "is_edge" in attr["data"]
            and attr["data"]["is_edge"] == "true"
        ):
            hadamards[name] = []
            continue
        ...
        if "data" in attr:
            d = attr["data"]
            if not "type" in d or d["type"] == "Z":
                g.set_type(v, VertexType.Z)
            elif d["type"] == "X":
                g.set_type(v, VertexType.X)
            elif d["type"] == "hadamard":
                g.set_type(v, VertexType.H_BOX)
            else:
                raise TypeError("unsupported type '{}'".format(d["type"]))
            if "value" in d:
    ...
    for name, attr in j.get("wire_vertices", {}).items():
        ...
        g.set_vdata(v, "name", name)
        names[name] = v
        if "input" in ann and ann["input"]:
            inputs.append(v)
        if "output" in ann and ann["output"]:
            outputs.append(v)
        # g.set_vdata(v, 'x', c[0])
        # g.set_vdata(v, 'y', c[1])
    ...
    for l in hadamards.values():
        if len(l) != 2:
            raise TypeError("Can't parse graphs with irregular Hadamard nodes")
        e = g.edge(*tuple(l))
...
def graph_to_json(g: BaseGraph[VT, ET]) -> str:
    ...
    for v in g.vertices():
        ...
        if not name:
            if t == VertexType.BOUNDARY:
                name = freenamesb.pop(0)
            else:
                name = freenamesv.pop(0)
        else:
            try:
        ...
        if t == VertexType.BOUNDARY:
            ...
            if t == VertexType.Z:
                ...
                raise Exception("Unkown vertex type " + str(t))
            phase = _phase_to_quanto_value(g.phase(v))
            if phase:
                node_vs[name]["data"]["value"] = phase
            if not node_vs[name]["data"]:
                del node_vs[name]["data"]
    ...
    for e in g.edges():
        ...
        if t == EdgeType.SIMPLE:
            ...
            node_vs[hadname] = {
                "annotation": {"coord": [(x1 + x2) / 2.0, (y1 + y2) / 2.0]},
    ...
    return json.dumps(
        {"wire_vertices": wire_vs, "node_vertices": node_vs, "undir_edges": edges}
    )
...
"""
    print(
        "Deprecated. Please use g.to_graphml() instead (where g is a Graph instance)."
    )
    ...
    for v in g.vertices():
        ...
        ).format(v, g.type(v), g.phase(v), g.row(v) * 100, g.qubit(v) * 100)
