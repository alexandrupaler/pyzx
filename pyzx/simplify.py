...
def clifford_simp(
    g: BaseGraph[VT, ET], quiet: bool = True, stats: Optional[Stats] = None
) -> int:
    ...
    def fuse_phases(self, i1: int, i2: int) -> None:
        ...
        if p1 == 0 or p1.denominator <= 2:  # Need to save non-Clifford location
            ...
            if v1 in self.phantom_phases:  # Already fused with non-Clifford before
                ...
                return
        if p1.denominator <= 2 or p2.denominator <= 2:
            raise Exception("Clifford phases here??")
...
def is_graph_like(g):
    ...
    for v1, v2 in itertools.combinations(g.vertices(), 2):
        ...
        if (
            g.type(v1) == VertexType.Z
            and g.type(v2) == VertexType.Z
