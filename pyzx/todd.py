...
class ParityPolynomial(object):
    ...
    def __str__(self) -> str:
        ...
        for t in sorted(self.terms.keys()):
            ...
            l.append(
                "{!s}{}".format(
                    val if val != 1 else "",
                    "@".join("x{:d}".format(v) for v in sorted(list(t))),
                )
            )
        return " + ".join(l)
    ...
    def to_par_matrix(self) -> Mat2:
        ...
        for par, val in self.terms.items():
            col: List[Union[Literal[0], Literal[1]]] = [
                1 if i in par else 0 for i in range(self.qubits)
            ]
            for i in range(val):
                cols.append(col)
    ...
    def add_parity(self, par: Iterable[int], subtract: bool = False) -> None:
        ...
        for a in range(len(p)):
            ...
            if v1 not in self.l:
                ...
                self.l[v1] = (self.l[v1] + mult) % 8

            for b in range(a + 1, len(p)):
                v2 = p[b]
                if (v1, v2) not in self.q:
                    self.q[(v1, v2)] = 1 if subtract else 3
                else:
                    self.q[(v1, v2)] = (self.q[(v1, v2)] - mult) % 4

                for c in range(b + 1, len(p)):
                    v3 = p[c]
                    if (v1, v2, v3) not in self.c:
                        self.c.add((v1, v2, v3))
                    else:
                        self.c.remove((v1, v2, v3))
...
def phase_gates_to_poly(
    gates: List[Gate], qubits: int
) -> Tuple[ParityPolynomial, List[ParitySingle]]:
...
def find_todd_match(
    m: Mat2,
) -> Tuple[int, int, Optional[List[Literal[0, 1]]], Optional[List[Literal[0, 1]]]]:
    ...
    with tempfile.NamedTemporaryFile(suffix=".gsm") as f:
        ...
        if USE_REED_MULLER:
            output = subprocess.check_output(
                [*settings.topt_command, "gsm", fname, "-a", "rm"]
            )
        else:
        ...
        # print(out)
    rows = (
        out[out.find("Output gate") : out.find("Successful")].strip().splitlines()[2:]
    )
...
def todd_simp(
    gates: List[Gate], qubits: int, quiet: bool = True
...
def todd_on_graph(g: BaseGraph[VT, ET]) -> None:
    ...
    for par, (_, v) in gadgets.items():
        ...
        phase = g.phase(v)
        for i in range(phase.numerator):
            cols.append(col)
