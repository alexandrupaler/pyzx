...
def hadamard_simp(
    ...
    while True:
        ...
        if len(m) == 0:
            break
        i += 1
        if i == 1 and not quiet:
            print("hadamard_simp: ", end="")
        if not quiet:
            print(len(m), end="")
        ...
        if not quiet:
            print(". ", end="")
        if g.num_vertices() >= vcount:
            break  # To make sure we don't get in an infinite loop
...
def to_hypergraph_form(g: BaseGraph[VT, ET]) -> None:
    ...
    for e in es:
        if g.edge_type(e) == EdgeType.HADAMARD:
            ...
            qs = g.qubit(s)
            qt = g.qubit(t)
            if types[s] == VertexType.BOUNDARY or types[t] == VertexType.BOUNDARY:
                continue
            h = g.add_vertex(VertexType.H_BOX)
            del_e.append(e)
            ...
            g.scalar.add_power(
                -1
            )  # Correct for sqrt(2) scalar difference in H-edge and H-box.
            if qs == qt:
                g.set_qubit(h, qs)
            else:
                q = (qs + qt) / 2
...
def copy_simp(
    ...
    return simp(
        g, "copy_simp", match_copy, apply_copy, matchf=matchf, quiet=quiet, stats=stats
    )
...
def hbox_parallel_not_remove_simp(
    g: BaseGraph[VT, ET],
    matchf: Optional[Callable[[VT], bool]] = None,
    quiet: bool = False,
    stats: Optional[Stats] = None,
    ...
    return simp(
        g,
        "hbox_parallel_not_remove_simp",
        match_hbox_parallel_not,
