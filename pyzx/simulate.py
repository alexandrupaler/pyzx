...
class SumGraph(object):
    ...
    def full_reduce(self, quiet: bool = True) -> None:
        ...
        for i, g in enumerate(self.graphs):
            ...
            if not g.scalar.is_zero:
                terms.append(g)
            elif not quiet:
                print("Graph {:d} is zero".format(i))
        self.graphs = terms
    ...
    def inner_product_with_random_state(self) -> complex:
        ...
        for g in terms:
            ...
            g.set_outputs(())
            g.add_edges(
                [(vs[i1], vs[i2]) for (i1, i2) in connections], EdgeType.HADAMARD
            )
            ...
            if g.num_vertices() != 0:
                raise Exception("Diagram wasn't fully reduced")
    ...
    def sample(
        ...
        if post_selected is not None:
            gsum = self.post_select(post_selected)
        else:
            gsum = self
        gsum.full_reduce()
        ...
        if post_selected is not None:
            for (
                ...
                qubits
            ):  # If we post-select, this changes which qubit points to which output
                qubit_map[q] -= sum(1 for v in post_selected if v < q)
        norm = gsum.estimate_norm(epsilon)
        if not quiet:
            print("Estimated original norm:", norm)
        if norm < 0.01 and not quiet:
            print(
                "Norm very close to zero. Possibly post-selected to zero probability event?"
            )
            ...
            for q in qubits:
                ...
                if val == 0:
                    if temp is not None:
                        ...
                        current = current.post_select({qubit_map2[q]: "0"})
                    prevprob *= prob
                else:
...
def calculate_path_sum(g: BaseGraph[VT, ET]) -> complex:
    ...
    for v in g.vertices():
        if v in variable_dict:
            continue
        if not phases[v]:
            continue  # It is the axle of a phase gadget, ignore it
...
def replace_magic_states(g: BaseGraph[VT, ET], pick_random: Any = False) -> SumGraph:
    ...
    outputs = g.outputs()
    for v in g.vertices():
        if not phases[v] or phases[v].denominator != 4:
            continue
    ...
    if not pick_random:
        candidates = sorted(ranking.keys(), key=lambda v: ranking[v], reverse=True)[
            :num_replace
        ]
    else:
...
def replace_phi1(g: BaseGraph[VT, ET], verts: List[VT]) -> BaseGraph[VT, ET]:
    ...
    w1, w2, w3, w4, w5 = ws
    g.add_edges(
        [g.edge(*e) for e in [(w1, w3), (w1, w4), (w2, w4), (w2, w5), (w3, w5)]],
        EdgeType.HADAMARD,
    )
    return g
...
def replace_2_S(g: BaseGraph[VT, ET], verts: List[VT]) -> BaseGraph[VT, ET]:
    w = g.add_vertex(
        VertexType.Z, g.qubit(verts[0]) - 0.5, g.row(verts[0]) - 0.5, Fraction(1, 2)
    )
